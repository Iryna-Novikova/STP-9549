...
function initPagination(totalDots, active) {
  ...
  for (let i = 0; i < totalDots; i++) {
    dots += ` 
      <span class="pgntn-dot" data-pgntn-span='testimon' data-active-dot='${
        i === active
      }' >
      </span>`;
  }
...
function updateSlide() {
  ...
  dotsElm.forEach(dotElm => {
    dotElm.dataset.activeDot = 'false';
  });
  dotsElm[activeIndex].dataset.activeDot = 'true';
  // swiper.isActive = true;

  if (activeIndex === 0) {
    refer.PrevBtnElm.disabled = true;
  }
  if (activeIndex === swiper.slides.length - 1) {
    refer.NextBtnElm.disabled = true;
  }
}
